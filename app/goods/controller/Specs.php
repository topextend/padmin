...
// |----------------------------------------------------------------------
// |Date         : 2021-02-27 14:22:10
// |----------------------------------------------------------------------
// |LastEditTime : 2021-02-27 17:33:42
// |----------------------------------------------------------------------
...
class Specs extends Controller
{
    ...
    {
        if (empty($data['spec_code'])) {
            $data['spec_code'] = CodeExtend::uniqidDate(12, 'S');
        }
        if ($this->request->isPost()) {
            ...
            if (!isset($data['id'])) {
                ...
                if ($this->app->db->name($this->table)->where($where)->count() > 0) {
                    $this->error("类型{$data['spec_name']}已经存在，请使用其它模板名称！");
                }
            }
        }
    }
